...
function transformPostsToSearchObjects(articles) {
  const transformed = articles.map((article) => {
    return {
      ...
      lastUpdateDate: article.data.lastUpdateDate,
      type: 'article'
    };
  });
...
// const AUTHOR_PATH = path.join(process.cwd(), 'posts');
...
// const TAG_PATH = path.join(process.cwd(), 'tags');
...
(async function () {
  ...
  try {
    ...
    const index = client.initIndex('matheusmazeto');
