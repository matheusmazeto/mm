...
> Using object literals always gives you new, **unique** objects, while primitive values (except for numbers, they are complicated) are **reused**. That means with `const a = 'foo'; const b = 'foo'`, we only have one string `foo` stored in the memory at runtime due to [string interning](https://en.wikipedia.org/wiki/String_interning). This might seem strange to you at first, and if you need proof, please take a look at [my post](https://www.matheusmazeto.com/posts/javascript-memory) on the topic of the memory model in V8.
...
## Further Reading
- [JavaScript memory model demystified](https://www.matheusmazeto.com/posts/javascript-memory)
